...
```{r means-reporting}
mean.pollard <- mean(richness.df$`Pollard Walk`)
mean.malaise <- mean(richness.df$Malaise)
```
Mean species richness in Pollard walks: `r round(x = mean.pollard, digits = 3)`

Mean species richness in Malaise traps: `r round(x = mean.malaise, digits = 3)`
...
ggsave(filename = "output/richness-pairwise-boxplot.png", plot = richness.boxplot)
```